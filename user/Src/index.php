...
<div class="container center" >
	...
	<?php 
		...
		if($sql2){
			...
			if($featured["is_banned"]!=1 && $featured["is_deleted"]!=1){
			?>
				<h3 style="font-family: 'Lemonada', cursive;">Featured Blog</h3>
				<div class="card">
				    <div class="row no-gutters card-row">
				        <div class="col-sm-6 card-image" >
				        	<?php if($featured['blog_covpic']){ ?>
								<img src="./Images/Cover_Images/<?php echo $featured['blog_covpic']; ?>" class="cover-img" alt="<?php echo $featured['blog_title']; ?>">
				        	<?php }else{ ?>
								<img src="./Images/noimage.jpeg ?>" class="cover-img" alt="<?php echo $featured['blog_title']; ?>">
				        	<?php } ?>
				        </div>
				        	...
				        	<div class="card-body" style="text-align: left;">
				        		<div class="row card-head">
				        			<div class="col-sm-6 card-head-left">
				        					<?php 
												$sql1=$obj->conn()->query("SELECT * FROM users WHERE user_id='$featured[blog_by]'");
				        						$users=$sql1->fetch_assoc(); ?>
				        						<?php 
									          		if($_SESSION['user_id']==$featured['blog_by']){ ?>
												<a href="./profile.php" >
								          			<?php }else{ ?>
								          		<a href="./other_profile.php?id=<?php echo $featured['blog_by'] ?> " >	
									          		<?php }
									          		?>
				        							<?php 
					        						if(!$users['user_pic']){ ?>
				        			...
				        			<div class="col-sm-6 card-head-right" style="text-align: right;">
				        				<span class="text-muted" style="text-transform: uppercase;"><?php echo $featured['blog_cat']; ?></span><br>
				        				...
				        				<?php 
											...
											$lst_updt_time = new DateTime( $featured['lst_updt_time']);
											...
											if($minutes < 60){
												...
												$time =date("d M Y",strtotime($featured['lst_updt_time']));
				        		...
				        		<div class="card-main">
				        			...
				        			<div class="row card-middle" style="height: 121px">
				        				<div class="col-sm-12" >
				        					<div class="card-text">
				        						<?php
				        						if(strlen($featured['blog_content'])>265)
				        							echo substr($featured['blog_content'],0,265)."..."; 
				        						else
				        							echo substr($featured['blog_content'],0,265); 
				        						?>
				        					</div>
						        	...
						        	<div class="card-readmore">
										<div class="row section">
										  <div class="col" style="text-align: right;">
										    <!-- Modal Trigger -->
											<a href="" class="view" style="color:blue" data-bs-toggle="modal" data-bs-target="#readMoreModal" id="<?php echo $featured['blog_id'] ?>" data-user-id="<?php echo $_SESSION['user_id']  ?>" >
												Read More
			                    ...
			                    <div class="row card-footer" id="card-footer">
			        				<div class="col-sm-2">
											<?php 
											$obj1= new Config;
											$result =$obj1->conn()->query("SELECT * FROM likes WHERE blog_id='$featured[blog_id]' ");
											$row=$result->num_rows;
											$userliked=$obj1->conn()->query("SELECT * FROM likes WHERE blog_id='$featured[blog_id]' AND liked_by='$_SESSION[user_id]' ");
											$isliked=$userliked->num_rows;
											if($isliked==1){ ?>
												<a class="btn-flat like" style="color:red;" id="<?php echo $featured['blog_id'] ?>" data-user-id="<?php echo $_SESSION['user_id']  ?>">
											<?php }else{ ?>
												<a class="btn-flat like" style="color:black;" id="<?php echo $featured['blog_id'] ?>" data-user-id="<?php echo $_SESSION['user_id']  ?>">
											<?php }?>
											<i class="fas fa-heart"></i>
	...
	<h3 style="font-family: 'Lemonada', cursive;">Blogs</h3>
	<div class="row" id="vcard">
	  	<?php
			$obj= new Config;
			// echo $_SESSION['user_id'];
			$sql=$obj->conn()->query("SELECT * FROM blog WHERE (is_draft=0 OR is_draft IS NULL) AND is_published=1 AND (is_approved=1) AND (is_banned=0 OR is_banned IS NULL) AND (is_deleted=0 OR is_deleted IS NULL) ORDER BY lst_updt_time DESC");
		...
		<?php }
			...
			echo "<h4 style='font-family: 'Lemonada', cursive;'>No blogs found</h4>";
		}
	 	?>
